# gens.team - Claude Code 配置

> AI 助手行为配置，指导 Claude Code 在本项目中的决策和行为。

## 项目概述

**gens.team** - 企业级 AI 深度研究和内容管理平台。
...
6. **品牌名称**: 禁止硬编码品牌名（"gens.team"/"Raven"/"DeepDive"等），前端用 `config.brand.*`（from `@/lib/utils/config`），后端用 `APP_CONFIG.brand.*`（from `common/config/app.config`）。Logo 用前端 `<BrandLogo />` 组件或后端 `BrandLogoService`
