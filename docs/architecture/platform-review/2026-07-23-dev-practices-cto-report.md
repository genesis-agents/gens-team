# 如何构建一个 Agent / 多 Agent 系统 — gens-team（原 GenesisPod）实践复盘（CTO 汇报）

> **日期**: 2026-07-23（v5.1：项目更名 gens-team,品牌域名 gens.team,文中旧名 GenesisPod 仅存于历史文档引用；v5：按"内部技术探索 + 五视角×三句"重构,主线收敛为 Agent/多 Agent 系统构建）
...
**gens-team（原 GenesisPod,仓库已更名 `genesis-agents/gens-team`,域名 gens.team）是一个内部技术探索项目,不是对外销售的产品。** 它的探索命题只有一个：**如何构建一个可信的 Agent / 多 Agent 系统**——包括 Agent 运行时（编排、记忆、守护、追踪）、多 Agent 协作（分工、审校、交接）、以及支撑这一切的工程方法论。
