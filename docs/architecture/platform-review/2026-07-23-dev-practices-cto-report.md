...
> **日期**: 2026-07-23（v3：补"为什么做这个系统"立论章 + 验收 rubric 风险入台账；v2：按五视角重组）
...
**为什么做这个系统**：AI Agent 市场两端已经挤满——左端是要用户自己拼装的零件市场（MCP/技能库，面向开发者），右端是全能但失控的通用自主 Agent（"龙虾"类，25 万 star 的势头与账户盗刷/删文件的事故并存）。GenesisPod 押注中间唯一别人做不好的位置：**面向一人公司/专业个体，卖"干完的活"——开箱即用、分工有审校、有交付标准的专家 Agent，交付可直接给客户的专业成果**。这个定位决定了本报告后面的一切：产品承诺是"可信、一致、可问责"，所以平台自身的工程治理必须重到同等程度（立论详见 §1）。
...
> 权威来源：`docs/architecture/product-positioning.md`（2026-06-09 落档的定位北极星，统领产品方向/护城河/信息架构）。

### 1.1 商业立论：市场的空位在"可信的专业交付"
...
### 1.3 治理强度是产品命题的镜像（为什么这个项目的工程实践值得汇报）

- 本报告后面那些"重"的实践（34 个架构 spec、10 步 pre-push、断路器、成本护栏）不是工程洁癖：产品承诺"可信、一致、可问责",平台自身若不可信,承诺不成立。6-21 失控事故直接打脸 JTBD 里的"质量稳定、出问题能复盘"——所以断路器与成本上限是产品命题的一部分,而非运维细节（呼应 §2.2）。
...
**定位**：
1. 先定交易单位再定架构：卖工具、卖 agent、还是卖"干完的活"？交易单位决定平台要自建哪些层（本项目因卖"可问责的交付"而必须自建 harness 层与验收体系）。

**产品**：
2. 需求进场必须转成可验证目标（测试命令/指标+基线+目标值）；答不出验收标准的需求退回澄清。
3. AI 功能的成本/时长上限是 PRD 参数,不是工程默认值；"自动继续"类功能必须定义快停条件。

**架构**：
4. 每条架构规则必须回答"谁在机器上拦截它"；ESLint（写时）+ jest spec（推前）+ CI 合并门（合并时）三层执行点缺一不可。
5. 上提公共层设消费方门槛（≥2 个真实 import），用测试拦住空壳抽象；顶层目录只用业界标准词。
6. 敢做减法：合并数据库、删除 0 消费方的层,并把反向决策连理由一起留档。

**工程**：
7. 合并门用 `always()` + 显式逐 job result 检查,防 skip/cancel 被误判为通过。
8. 用反向证据测试验证守护本身会 fire；审计脚本要能在反模式样本上自证非假绿。
9. 断路器同时覆盖"冻死"（无心跳）与"空转"（高频事件但无进展 + 成本速率），单任务设成本上限。

**演进**：
10. 大重构拆小波次、按风险梯度排序、每波以架构 spec 全绿收口；配置迁移用 dual-read 过渡。
11. 存量违规用棘轮基线冻结（只减不增），让新增零容忍与历史债务解耦；honor-only 规范进显式债务清单并排期升级。

**AI 协作**：
12. Sub-agent 一律文件白名单 + worktree 隔离；全局回退命令永久禁用。
13. AI 产出的架构结论必须过对抗性复核；显式维护"已证伪/勿追误报"清单。
